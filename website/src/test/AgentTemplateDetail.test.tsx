...
describe('the crew\'s own copy bound', () => {
  ...
  it('disables Reset when the copy matches its origin (nothing to reset)', async () => {
    mockApi.agentsInstalled.mockResolvedValue(OWN_COPY_INSTALLED)
    // Identical specs -> zero changes -> Reset inert, no change pill.
    const same = { name: 'x', model: 'opus', prompt: 'same', skills: ['grill'] }
    detailsByName({ 'atlas-crewA': same, atlas: same })
    renderPane({ template: 'atlas-crewA', crew: 'crewA' })

    const reset = await screen.findByRole('button', { name: 'Reset my changes' })
  ...
  it('Reset confirms, then rebinds to the origin and removes the copy', async () => {
    ...
    // Reset is inert until the live diff against the origin has resolved; wait
    // for it to enable so the click actually opens the confirm.
    const reset = await screen.findByRole('button', { name: 'Reset my changes' })
