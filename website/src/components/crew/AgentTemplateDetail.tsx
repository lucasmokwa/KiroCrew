...
export default function AgentTemplateDetail({
  ...
  // user step), rebind the parent's dropdown, write the edit to the copy. The
  // endpoint is idempotent, so a re-entrant call cannot fork twice.
  const forkIfNeeded = async (): Promise<string> => {
    if (!crew || isOwnCopy) return template
  ...
  const opts = options.includes(template) || !template ? options : [template, ...options]
    ...
    <div className="mt-1 rounded-[10px] border border-border-strong">
      ...
      <div className="sticky top-0 z-[3] flex flex-wrap items-center gap-x-2.5 gap-y-1 rounded-t-[10px] border-b border-border-strong bg-bg-elevated px-3.5 py-2">
        ...
        {isOwnCopy && changes.length > 0 && (
          <Popover open={changesOpen} onOpenChange={setChangesOpen}>
            <PopoverTrigger asChild>
              <button
                type="button"
                className="cursor-pointer rounded-full border border-accent/40 bg-accent/10 px-2 py-px text-[10.5px] text-accent"
              >
        ...
        {isOwnCopy && (
          <span className="ml-auto flex items-center gap-3.5 whitespace-nowrap">
            {/* Recovery, not a primary action — quiet on purpose, and inert
                when there is nothing to reset. */}
            <button
              type="button"
              onClick={resetToOrigin}
              disabled={!changes.length}
              className="cursor-pointer border-0 bg-transparent p-0 text-[11.5px] text-muted enabled:hover:text-text disabled:cursor-default disabled:opacity-50"
            >
      ...
      <div className="px-4 pb-4">
        ...
        {!template ? null : isLoading ? (
          ...
          <p className="m-0 mt-4 text-[12px] text-warn">{i18nT('components.agentTemplateDetail.could_not_load')}</p>
        ) : (
          <>
            ...
            <div className="max-w-[320px]">
              <SimpleSelect
                ...
                value={detail.model || ''}
                onChange={m => patchModel.mutate(m)}
      ...
      <Dialog open={publishOpen} onOpenChange={o => { setPublishOpen(o); if (!o) setPublishError('') }}>
        <DialogContent className="max-w-[420px]">
          ...
          {publishError && <p className="m-0 mt-1 text-[11.5px] text-danger">{publishError}</p>}
