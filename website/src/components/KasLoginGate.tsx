  ...
  return (
    ...
    >
      ...
      <div className="mt-6 flex flex-wrap items-center gap-2">
        <Btn type="button" primary onClick={() => openAuthTab(session.auth_url)}>
          <ExternalLink className="lucide-inline" />
          {i18nT('components.kasLogin.loopback_open_again')}
        </Btn>
        <Btn type="button" onClick={onUseCode}>
      ...
      <button
        type="button"
        onClick={onCancel}
        className="mt-5 cursor-pointer text-[13px] text-muted underline-offset-2 hover:text-text hover:underline focus-ring"
      >
...
export default function KasLoginGate({ children }: { children?: ReactNode }) {
  ...
  useEffect(() => {
    if (session?.kind !== 'loopback') return
  ...
  }, [session, pollStatus, pollCode])

  const status = statusQuery.data

  const reset = () => {
    if (session) void api.kasLoginCancel(session.login_id).catch(() => undefined)
    setSession(null)
    setFellBack(false)
    beginDevice.reset()
    beginLoopback.reset()
  ...
  const useCodeInstead = () => {
    if (!session) return
    const provider = session.provider
    void api.kasLoginCancel(session.login_id).catch(() => undefined)
    setSession(null)
    setFellBack(true)
    beginDevice.mutate({ provider })
  }
