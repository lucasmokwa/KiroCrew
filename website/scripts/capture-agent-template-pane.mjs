 ...
 *              live change-count pill (opened for its own frame), Reset +
 *              Save-as-new-template actions
 *
...
try {
  for (const [crew, template, expectBadge, expectOwnCopy] of [
    ...
    if (expectOwnCopy) {
      ...
      for (const want of [/Reset my changes/, /Save as new template/]) {
        if (!(await sheet.getByText(want).count())) {
          throw new Error(`${crew}: own-copy header is missing ${want}`)
        }
      }
