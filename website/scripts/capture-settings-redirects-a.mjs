...
// ── A4 VoiceDisabledModal ────────────────────────────────────────────────────
await scene('A4', async () => {
  const o = await openPage({
    override: (u, route, json) => {
      // STT switched on but the provider's packages are missing → 'unavailable'
      // reason → "Pick an installed provider under Settings → Voice…".
      if (u.pathname === '/api/config/stt') return json({ ...STT_CONFIG_OK, enabled: true, available: false, provider: 'local' }), true
      if (u.pathname === '/api/stt/status') return json({ available: false, code: 'missing_packages', detail: '', models: [] }), true
      return false
    },
  })
  const { page } = o
  ...
  await mic.click()
  const fragment = 'Pick an installed provider under'
  await shoot(page, {
    file: 'a4-voice-disabled-modal.png', fragment,
    container: page.getByRole('dialog').filter({ hasText: fragment }).first(),
  })
  return o
})

// ── A1 Ops Mission Control → Settings → Desktop notifications card ───────────
await scene('A1', async () => {
  const OMC = '/api/apps/ops-mission-control'
  const rotation = { on_shift: true, who: 'you', until: '', unknown: false, armed: {}, identities: { login: 'you' } }
  const ledgerStats = { total: 0, verified: 0, high_confidence: 0, total_uses: 0, proven: 0, demoted: 0, total_misses: 0 }
  const state = {
    incidents: [], counts: {}, blocked: {}, providers: [], rotation, ledger: ledgerStats,
    notify: {
      enabled: true, bus_available: true, ready: true, detail: '',
      channels: [
        { id: 'incident-opened', name: 'Incident opened', icon: 'siren', default_priority: 'critical' },
        { id: 'incident-resolved', name: 'Incident resolved', icon: 'check', default_priority: 'passive' },
      ],
    },
    companions: [],
  }
  const o = await openPage({
    override: (u, route, json) => {
      if (!u.pathname.startsWith(OMC)) return false
      const p = u.pathname.slice(OMC.length)
      if (p === '/state') return json(state), true
      if (p === '/providers') return json({ providers: [] }), true
      if (p === '/rotation') return json(rotation), true
      if (p === '/ledger') return json({ entries: [], stats: ledgerStats }), true
      if (p === '/signals') return json({ signals: [], sources: [], health: [] }), true
      if (p === '/incidents') return json({ incidents: [] }), true
      if (p === '/proposals') return json({ proposals: [], total: 0 }), true
      if (p === '/handover') return json({ incidents: [], generated_at: '' }), true
      return json({}), true
    },
  })
  const { page } = o
  await page.goto(`${base}/ops-mission-control`, { waitUntil: 'networkidle' })
  // The in-page SegmentedControl renders plain buttons, and the sidebar nav also
  // has a "Settings" button earlier in DOM order — take the LAST match so the
  // click lands on the app's own view switcher, not the dashboard nav.
  const tab = page.getByRole('button', { name: /^settings$/i }).last()
  await tab.waitFor({ state: 'visible', timeout: 15_000 })
  await tab.click()
  const fragment = 'To silence one of these'
  const text = page.getByText(fragment, { exact: false }).first()
  await text.waitFor({ state: 'visible', timeout: 15_000 })
  await shoot(page, {
    file: 'a1-omc-desktop-notifications-note.png', fragment,
    // The enclosing settings Card: nearest ancestor that also holds the card title.
    container: text.locator('xpath=ancestor::*[.//*[contains(normalize-space(.), "Desktop notifications")]][1]'),
  })
  return o
})
