 ...
 * (Capabilities → MCP / the Deploy page / Capabilities → Skills).
 *
...
const ACCOUNTS_EMPTY = { supported: true, accounts: [], totals: { accounts: 0, profiles: 0, profilesHealthy: 0 } }
const PROFILES_UNSUPPORTED = { supported: false, profiles: [], max: 20, registeredCount: 0 }
...
async function answer(route) {
  ...
  // ---- AWS Control
  if (path.startsWith('/api/apps/aws-control/')) {
    const app = path.slice('/api/apps/aws-control'.length)
    if (app === '/accounts') return json(route, ACCOUNTS_EMPTY)
    if (app === '/profiles/available') return json(route, PROFILES_UNSUPPORTED)
...
// ---- C4 + C5: AWS Control, zero accounts + unsupported profile listing -----
{
  const page = await newPage()
  try {
    await page.goto(`${base}/aws-control`, { waitUntil: 'domcontentloaded' })
    await page.waitForTimeout(800)
    await shoot(page, 'C4', 'c4-aws-control-empty.png',
      'Once one is set up, its account appears here',
      page.locator('[data-testid="accounts-empty"]'))
  } catch (e) { skip('C4', String(e.message || e).split('\n')[0]) }
  try {
    await shoot(page, 'C5', 'c5-aws-control-unsupported.png',
      'list local AWS profiles automatically',
      page.locator('[data-testid="add-accounts"]'))
  } catch (e) { skip('C5', String(e.message || e).split('\n')[0]) }
  await page.close()
}
