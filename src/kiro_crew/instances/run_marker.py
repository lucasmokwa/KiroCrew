...
def pid_start_token(pid: int) -> str:
    ...
    A thin, fail-closed adapter over
    :func:`kiro_crew.platform_compat.get_process_start_id` -- the single
    producer of both the value the sidecar records and the value a reader
    compares it against, so the two can never drift in how they spell it. That
    helper is documented colon-free and single-token, is in-process on every
    platform (no subprocess), and on macOS reads ``libproc`` at MICROSECOND
    resolution: ``process_start_time``'s ``ps -o lstart=`` spelling is only
    1-second granular there, so a pid recycled inside the same second would
    reproduce an identical token and the guard would silently pass.
    ...
    match -- notably on a Windows host, where there is no implementation at all.
    """
    try:
        return platform_compat.get_process_start_id(int(pid)) or ""
