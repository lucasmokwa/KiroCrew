...
async def relay_remote_turn(
    ...
    *chunks* exists for tests: pass an async byte iterator to drive the replay
    ...
    try:
        ...
        #
        # Clear the in-flight marker set at the top FIRST, so the save records a
        # turn that finished: whether it completed, truncated or errored, the tail
        # is now in the local window and there is nothing for a reload to recover.
