    ...
    * the ceiling currently INSTALLED grants break-glass to the ``env`` tier and that
    ...
    for candidate in (TIER_ENV, TIER_HOME):
        ...
        if candidate == TIER_ENV:
            ...
            continue
        if env_present:
            # The env document outranks home, so home is not the subordinate and its
            # grant cannot stand a refresh down. A granted bundled document sits between
            # them but is packaged, so shape 1 has already covered it.
            break
