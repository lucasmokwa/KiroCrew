...
def _absence_could_be_self_inflicted(path: Path) -> bool:
    """Could THIS account have MADE the managed document absent?

    Only a yes justifies refusing. The guard exists to tell deletion apart from "never
    provisioned", and that distinction only exists where a deletion could be attributed
    to this account -- so every "cannot tell" answer has to read as ordinary absence
    rather than as an accusation, or the guard takes down hosts it was never about.

    Two cases answer "cannot tell":

    * **Off POSIX.** ``path_writable_by_current_user`` returns ``True`` there
      unconditionally -- it is a "permissions are unreadable here" sentinel, not an
      observation -- so honouring it refuses every Windows host. Windows has no managed
      tier in production (``_managed_policy_path`` hands back no path at all), and a test
      that points the accessor at a macOS path on a Windows runner must not abort the
      loader.
    * **UID 0.** Root can write every directory on the host, so "writable" carries no
      information about whether a STANDARD user removed the file. Refusing there aborted
      startup for every root-run standalone install: a self-inflicted crash in the guard,
      not a governance signal.

    A failure INSIDE the predicate is "cannot tell" as well. It walks ancestors and stats
    them, and an ``OSError`` from that walk is not evidence of write access -- letting it
    escape would turn an unreadable directory, or off-POSIX path semantics, into a boot
    abort. The predicate's own docstring already skips un-stattable components for the
    same reason; this is that rule applied to the call as a whole.
    """
    from kiro_crew.platform_compat import IS_POSIX, path_writable_by_current_user
    ...
    if getuid is not None and getuid() == 0:
        return False
    try:
        return bool(path_writable_by_current_user(path))
    except OSError:
        logger.debug(
            "managed directory writability is unreadable for %s; treating the absence "
