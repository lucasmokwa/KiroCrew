...
class KasLoginService:
    ...
    async def _run_loopback(
        ...
        ``_persist_and_finish`` only while the login is still registered, so a
        cancelled login ("use a code instead", start over) can never leave a
        credential behind that the user did not see land.
        """
        try:
    ...
    async def _poll_loopback(self, login_id: str, pending: _PendingLoopbackLogin) -> dict[str, Any]:
        ...
        # Claim the entry atomically: two concurrent polls of the same finished
        # task must not both persist. Whoever pops it owns the terminal answer.
        async with self._lock:
            owned = self._pending.pop(login_id, None) is not None
        if not owned:
            raise UnknownLoginError(login_id)
