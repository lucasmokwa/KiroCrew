...
def _write_plist_atomic(contents: str) -> None:
    ...
    on the same filesystem, so a SIGINT or crash mid-write leaves either the
    old plist or no plist at all — never a partial XML document that
    ``launchctl load`` would reject. The shared helper also cleans the temp
    file up on ``BaseException`` (so that SIGINT leaves no scratch file) and
    retries the rename past a Windows sharing violation. ``fsync`` is off.

    ``mode=0o600`` has to be explicit: with no *mode*, ``atomic_write``
    applies the umask default and would publish the plist world-readable.
    launchd loads the agent as the owning user, so nothing else needs to
    read it.
    """
