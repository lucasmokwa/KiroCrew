...
def make_external_id(profile: str) -> str:
    """A publish handle binding an unguessable key to the PROFILE NAME it was published under.

    It binds the NAME, not the account behind it, and the difference is a real exposure
    rather than a nicety. A profile name is a local alias the owner can repoint at another
    account by editing their AWS config. Do that, then delete the artifact, and the
    withdrawal resolves the name to the NEW account, finds no drive there or the wrong
    one, reports a removal that removed nothing, and the record is cleared -- leaving the
    original copy public in the old account with its handle erased. That is the precise
    harm this module is otherwise built to prevent.

    Binding the ACCOUNT ID instead is the fix, and it is not done here because it changes
    what a handle IS: the separator is partitioned on the FIRST occurrence so a profile
    name may itself contain it (there is a test for exactly that), which makes appending a
    field ambiguous, and six parse sites read the current shape. It is recorded with the
    publication-existence contract rather than improvised. Until then, treat a repointed
    profile as a known way to strand a copy.
    """
...
class PersonalDriveProvider(PublishProvider):
    ...
    def __init__(self) -> None:
        ...
        an EXISTING publication pins the PROFILE NAME it was published under into its
        ``external_id``, so a moving default cannot drag it along. Pinning the name is
        weaker than pinning the account -- see ``make_external_id`` for the exposure that
        leaves open -- but it is what the current handle shape carries.
        """
    ...
    def _profile_for(self, external_id: str) -> tuple[str, str]:
        """Resolve the profile NAME an existing publication was published under.

        Every mutation path routes its account decision through here, which also makes
        this the one place an account check would belong once handles carry an account id.
        Today it resolves a name, so it answers "which account does this name point at
        NOW" rather than "which account holds this copy" -- see ``make_external_id``.
        """
...
def register_public_edition_providers() -> None:
    ...
    profile NAME it was published under into its ``external_id`` -- not the account behind
    that name, which the owner can repoint (see ``make_external_id``).
