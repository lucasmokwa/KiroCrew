...
| Kiro sign-in gate | KAS-mode sign-in without kiro-cli: Google/GitHub via loopback (local) or device code (remote), Builder ID and company SSO via device code | Full-screen gate when no Kiro credential is stored | `components/KasLoginGate.tsx` | `handlers/kas_login.py` | `GET /api/kas-login`, `POST /api/kas-login/device`, `POST /api/kas-login/loopback`, `POST /api/kas-login/poll`, `POST /api/kas-login/cancel`, `POST /api/kas-login/logout` |
