...
- `credential-exfil-printenv-aws` — `printenv` (as a command word) naming a secret-bearing variable (`AWS_SECRET*` / `AWS_SESSION*` / `AWS_SECURITY*` / `AWS_ACCESS*`); `printenv AWS_REGION` is not a match. `credential-exfil-env-grep-aws` — an environment dump (`env` / `printenv` / `set` / `export -p`, as a command word) **piped** through `grep`/`awk`/`sed` selecting the bare `AWS`/`AWS_` prefix or a secret-bearing variable, all within one shell statement. Selecting a named non-secret variable (`env | grep AWS_REGION`), a dump with no filter (`env | cut -d= -f1`), `env`/`set` as a substring of another word (`src/environment/`, `unset`), or a grep in a later statement is not a match. A **partial** secret name (`env | grep AWS_S`) is deliberately not a match either: this rule is a command-shape speed bump, and the value such a grep would print is caught by `redact_credentials` (AKIA/ASIA + high-entropy detection) before it reaches a chat surface, so widening the selector to "every non-allowlisted name" would re-create the false positives the rule was narrowed to remove. The always-on `_ENV_CRED_PATTERNS` keystone reuses the identical regex (`_ENV_DUMP_GREP_AWS_PATTERN`), so narrowing one tier never leaves the block standing on the other.
