...
process named by `gateway-<port>.pid` — `platform_compat.get_process_start_id()`,
the same producer session PIDs use, in-process on every platform and microsecond
resolution on macOS (`process_start_time`'s `ps -o lstart=` spelling is only
1-second granular there, so a PID recycled inside the same second would reproduce
an identical value).
