...
class TestPromptExpansionStaysOffTheEventLoop:
    ...
    def test_the_slash_get_expansion_runs_on_a_worker_thread(self, mock_sel, monkeypatch):
        """``asyncio.run`` drives the loop on the CALLING thread, so an expansion
        that reports that thread's id ran on the loop."""
        import kiro_crew.dashboard.chat_runner as cr

        real = cr._expand_prompt_mention
        threads: list[int] = []

        def _record(message, state, slot):
            threads.append(threading.get_ident())
            return real(message, state, slot)

        monkeypatch.setattr(cr, "_expand_prompt_mention", _record)
        s, sl = _ss()
        asyncio.run(_run_chat(s, sl, "/prompts get nonexistent"))

        assert threads, "the expansion was never reached"
        assert threading.get_ident() not in threads

    def test_no_call_site_expands_a_mention_inline(self):
        """The ``@mention`` site cannot be driven without a live session, so the
        rule is pinned statically over ``chat_runner``'s own source: a bare call is
        an on-loop call, whichever site adds it."""
        import kiro_crew.dashboard.chat_runner as cr
        ...
        calls = [
            ...
            and node.func.id == "_expand_prompt_mention"
        ]
        offloaded = [
            node
            for node in ast.walk(tree)
            ...
            and node.args[0].id == "_expand_prompt_mention"
        ]
        assert not calls, f"{len(calls)} call site(s) expand a mention on the event loop"
