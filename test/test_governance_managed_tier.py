...
class TestTheManagedDirectoryMustNotBeUserWritable:
    ...
    def test_off_posix_absence_is_inert_even_though_the_predicate_says_writable(
        ...
        monkeypatch.setattr(WRITABLE_PRED_PATH, lambda path: True)
        _point_managed(monkeypatch, tmp_path / "dev.kirocrew.plist")
        assert governance._read_managed_policy() is None

    @_POSIX_ONLY
    def test_absence_under_uid_zero_is_inert_because_root_writes_everything(
        self, monkeypatch, tmp_path
    ):
        """Root's write access carries no information about a STANDARD user's deletion.

        The guard distinguishes deletion from "never provisioned" by asking whether THIS
        account could have caused the absence. Root can write every directory on the
        host, so "writable" is true everywhere and the answer is uninformative -- which
        aborted startup for every root-run standalone install.
        """
    ...
    def test_a_non_root_posix_account_is_still_refused_under_the_same_stub(
        ...
        Both exemptions are narrow. With POSIX true and a non-zero uid, a writable
        managed directory is exactly the case the guard exists for, so it must still
        refuse -- otherwise the carve-outs have disabled the check rather than scoped it.
        """
    ...
    def test_an_oserror_from_the_predicate_reads_as_cannot_tell_not_as_write_access(
        ...
        The predicate walks ancestors and stats them; an ``OSError`` from that walk means
        the answer is unknown. Letting it escape turned an unreadable directory into a
        boot abort -- the same failure mode the predicate's own docstring avoids when it
        skips un-stattable components.
        """
        monkeypatch.setattr("kiro_crew.platform_compat.IS_POSIX", True)
        ...
        monkeypatch.setattr(WRITABLE_PRED_PATH, explode)
        _point_managed(monkeypatch, tmp_path / "dev.kirocrew.plist")
        assert governance._read_managed_policy() is None
